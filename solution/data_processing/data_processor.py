...
from pyspark.sql.functions import split, col, from_unixtime, date_format
from cleaning_functions import clean_numerical_col, clean_alphabetical_col
...
def clean_df(df: DataFrame) -> DataFrame:
    num_mappings = {
        ...
        'Magnetic variation degrees': 'mag_variaton'
    }
    char_mappings = {
    ...
    df = (
        df.withColumn("datetime", date_format(from_unixtime(col("datetime")), "yyyy-MM-dd'T'HH:mm:ss"))
            .withColumnRenamed("UT date", "datestamp")
